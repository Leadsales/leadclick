  ...
  render() {
    ...
    if (this.showOptions) {
      options = [
        index.h(
          'div',
          { class: 'title', style: { backgroundColor: this.bgcolor ? this.bgcolor : '#3F40C2', color: this.fontcolor ? this.fontcolor : '#ffffff' } },
          this.instructions,
        ),
        index.h(
          'div',
          { class: 'card' },
          this.buttons.map((btn, index$1, arr) => {
            let icon;
            if (btn === 'Instagram') {
            ...
            return index.h(
              'div',
              { class: 'complete_row' },
              index$1 === 0 && index.h('div', { class: 'space' }),
              index.h(
                'div',
                {
                  class: 'row_item',
                ...
                index.h('img', { class: 'image', src: icon }),
                index.h('p', { class: 'integration' }, btn),
              ),
              index$1 !== arr.length - 1 && index.h('hr', null),
              index$1 === arr.length - 1 && index.h('hr', { class: 'hidden' }),
            );
          }),
        ),
      ];
    ...
    return index.h(
      ...
      index.h(
        ...
        index.h('img', { class: 'image', src: index.getAssetPath(`./assets/whatsapp-icon.svg`) }),
        index.h('p', { class: 'cta-text' }, this.cta),
      ),
      index.h(
        'p',
        { class: 'by' },
        '\u26A1\uFE0Fby',
        ' ',
        index.h('a', { rel: 'dofollow', target: 'blank', class: 'leadsales_link', href: 'https://leadsales.io/' }, 'leadsales.io'),
      ),
    );
  }
