    ...
    (e.atRule = 0 === s.indexOf(AT_START)),
      e.atRule
        ? 0 === s.indexOf(MEDIA_START)
          ...
          : s.match(RX.keyframesRule) && ((e.type = types.KEYFRAMES_RULE), (e.keyframesName = e.selector.split(RX.multipleSpaces).pop()))
      ...
      .forEach(function (e) {
        ...
        n.length > 0 &&
          e.parsedSelector.split(',').forEach(function (e) {
            (e = e.trim()), r.push({ selector: e, declarations: n, specificity: computeSpecificity(), nu: t });
          }),
          t++;
      }),
    r
...
function reScope(e, t) {
  var r = e.template.map(function (r) {
    ...
    n = e.selectors.map(function (r) {
      return __assign(__assign({}, r), { selector: replaceScope(r.selector, e.scopeId, t) });
    });
...
function loadDocumentStyles(e, t) {
  return Array.from(e.querySelectorAll('style:not([data-styles]):not([data-no-shim])'))
    .map(function (e) {
        ...
        s.usesCssVars
          ? n
            ? ((o['s-sc'] = t = s.scopeId + '-' + this.count),
              ...
              this.count++)
            : ((s.styleEl = o), s.usesCssVars || (o.textContent = executeTemplate(s.template, {})), this.globalScopes.push(s), this.updateGlobal(), this.hostScopeMap.set(e, s))
          : (o.textContent = r),
        o
      );
    }),
